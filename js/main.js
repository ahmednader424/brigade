"use strict";
// Data Structures
var _a = require("./../js/containers/track.js"), Track = _a.Track, Tracks = _a.Tracks;
var _b = require("./../js/containers/battalion.js"), Battalion = _b.Battalion, Battalions = _b.Battalions;
var Brigades = require("./../js/containers/brigade.js").Brigades;
var Target = require("./../js/containers/target.js").Target;
var Engager = require("./../js/containers/engager.js").Engager;
var Engagements = require("./../js/containers/engagement.js").Engagements;
var util = require("util");
// Load dependencies
var L = require("leaflet");
var turf = require("../Plugins/turf/turf.min.js");
...
// Tiles Url
var baseTiles = configuration["tiles_base"];
var darkTiles = configuration["tiles_dark"];
var heightsTiles = configuration["tiles_heights"];
var roadTiles = configuration["tiles_roads"];
var borderTiles = configuration["tiles_borders"];
var nameTiles = configuration["tiles_names"];
var fullTiles = configuration["tiles_full"];
var dbConfig = {
...
var grid = L.simpleGraticule(gridOptions);
var polylineMeasure = L.control.polylineMeasure(polylineMeasureOptions);
var scaleControl = L.edgeScaleBar();
var baseLayer = L.tileLayer.wms(baseTiles);
var darkLayer = L.tileLayer.wms(darkTiles);
var heightsLayer = L.tileLayer.wms(heightsTiles);
var roadsLayer = L.tileLayer.wms(roadTiles);
var borderLayer = L.tileLayer.wms(borderTiles);
var namesLayer = L.tileLayer.wms(nameTiles);
var fullLayer = L.tileLayer.wms(fullTiles);
var visibleLayers = [baseLayer];
var map = L.map("mapid", {
...
// Brigade data
var brigadesContainer = Brigades.getInstance();
...
function loadInitialState() {
    ...
    con.query(sql, function (err, result) {
        if (err)
            throw err;
        for (var i = 0; i < result.length; i++) {
            brigadesContainer.addBrigade(result[i].Brigade_Name, result[i].Brigade_Lat / 3600, result[i].Brigade_Long / 3600, [], result[i].ID_Brigade);
        }
    ...
    con.query(sql, function (err, result) {
        ...
        sel.innerHTML = "";
        battalionCommands.forEach(function (element) {
    ...
    con.query(sql, function (err, result) {
        ...
        sel.innerHTML = "";
        highLevelCommands.forEach(function (element) {
            var opt = document.createElement("option");
            opt.value = element;
            opt.innerHTML = element;
            sel.appendChild(opt);
    ...
    con.query(sql, function (err, result) {
        ...
        sel.innerHTML = "";
        brigadeReplyCommands.forEach(function (element) {
            var opt = document.createElement("option");
            opt.value = element;
            opt.innerHTML = element;
            sel.appendChild(opt);
    ...
    con.query(sql, function (err, result) {
        ...
        for (var i = 0; i < result.length; i++) {
            if (result[i].Valid == 1) {
                ...
                tracksContainer.addTrack(result[i].TRK_No, result[i].TRK_Lat / 3600, result[i].TRK_Long / 3600, result[i].Serial, result[i].VX, result[i].VY, type, result[i].Speed, result[i].Alt, result[i].TQ, result[i].TRK_Size);
            }
        }
        ...
        con.query(sql, function (err, result) {
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                battalionsContainer.addBattalion(result[i].Bat_Name, result[i].Bat_Lat / 3600, result[i].Bat_Long / 3600, 0, result[i].ID_Bat_Real, type, result[i].Bat_Availability, result[i].Bat_Link_Status, result[i].Bat_Alert_Status, result[i].FK_ID_Bat_Par, result[i].Bat_Alt, result[i].HotMissiles, result[i].ColdMissiles, result[i].Bat_Launcher_Status);
            }
            var union = undefined;
            battalionParameters.forEach(function (element) {
        ...
        con.query(sql, function (err, result) {
            if (err)
                throw err;
            var selectedTarget = engagementsContainer.target;
            var selectedEngager = engagementsContainer.engager;
            if (selectedEngager != undefined && selectedEngager.constructor.name == "Battalion") {
                engagementsContainer.engager = battalionsContainer.getBattalionWithName(selectedEngager.name);
            }
            if (selectedTarget != undefined) {
                engagementsContainer.target = tracksContainer.getTrackWithName(selectedTarget.name);
            }
            engagementsContainer.drawSelectionCircles();
            engagementsContainer.resetBattalionEngagements();
            for (var i = 0; i < result.length; i++) {
                var bat = battalionsContainer.getBattalionWithId(result[i].FK_ID_Bat_Real);
                var track = tracksContainer.getTrackWithSerial(result[i].FK_ID_All_Track);
                if (bat != undefined && track != undefined) {
                    ...
                    if (battalionEngagementId == null) {
                        ...
                        if (engagementType == "broke") {
                            ...
                            engagementsContainer.setActionLine(bat, track, String(engagementType));
                        }
                    }
        ...
        con.query(sql, function (err, result) {
            ...
            if (selectedEngager != undefined && selectedEngager.constructor.name == "Brigade") {
                engagementsContainer.engager = brigadesContainer.getBrigadeWithName(selectedEngager.name);
            }
            if (selectedTarget != undefined) {
                engagementsContainer.target = tracksContainer.getTrackWithName(selectedTarget.name);
            }
            ...
            for (var i = 0; i < result.length; i++) {
                var brig = brigadesContainer.getBrigadeWithId(result[i].FK_brigade_id);
                var track = tracksContainer.getTrackWithSerial(result[i].FK_track_id);
...
function updateEngagementTable(currentEngagements) {
    var table = document.getElementById("batEngagementTable");
    table.innerHTML = "";
    for (var i = 0; i < currentEngagements.length; i++) {
        ...
        commandHighlevelDiv.innerHTML = "<p class=\"statusTableElement\"> " + highLevelCommands[eng.FK_ID_Comm_HL] + " </p>";
        ...
        commandBattalionDiv.innerHTML = "<p class=\"statusTableElement\"> " + (battalionCommands[eng.FK_ID_Comm_Bat] == undefined
            ? "-"
            : battalionCommands[eng.FK_ID_Comm_Bat]) + " </p>";
...
function updateBattalionStatusTable() {
    ...
    for (var i = 0; i < battalionsContainer.numberOfBattalions; i++) {
        ...
        linkStatus.innerHTML = "<p class=\"statusTableElement\" style=\"color: " + (bat.linkStatus == 0 ? "#ff3333;" : "#40ff00;") + "\"> " + (bat.linkStatus == 0 ? "OFF" : "ON") + " </p>";
        cell2.appendChild(linkStatus);
        var cell3 = row.insertCell(2);
        var availabilityStatus = document.createElement("div");
        availabilityStatus.innerHTML = "<p class=\"statusTableElement\" style=\"color: " + (bat.availability == 0 ? "#ff3333;" : "#40ff00;") + "\"> " + (bat.availability == 0 ? "O" : "R") + " </p>";
...
function updateSystemModes(systemMode) {
    var table = document.getElementById("systemModeTable");
    var assignValues = {
        0: "Manual",
        1: "Auto",
        2: "Semi",
    ...
    var centralizedValues = {
        0: "Central",
        1: "Decentral",
    };
    var airRaidWarningValues = {
        0: "red",
        1: "yellow",
        2: "white",
    };
    ...
    assign.innerHTML = "<p class=\"statusTableElement\"> " + assignValues[systemMode[0].assign] + " </p>";
    assignCell.appendChild(assign);
    var weaponsCell = row1.insertCell(i++);
    var weapons = document.createElement("div");
    weapons.innerHTML = "<p class=\"statusTableElement\"> " + weaponsValues[systemMode[0].weapons] + " </p>";
    weaponsCell.appendChild(weapons);
    var defconCell = row1.insertCell(i++);
    var defcon = document.createElement("div");
    defcon.innerHTML = "<p class=\"statusTableElement\"> " + defconValues[systemMode[0].defcon] + " </p>";
    defconCell.appendChild(defcon);
    var centralizationCell = row1.insertCell(i++);
    var centralization = document.createElement("div");
    centralization.innerHTML = "<p class=\"statusTableElement\"> " + centralizedValues[systemMode[0].centralization] + " </p>";
    ...
    airRaidWarningDiv.innerHTML = "<div style=\"background-color: " + airRaidWarningValues[systemMode[0].air_raid_warning] + "; width: 50px; height: 50px; margin: 10px;\"> </div>";
    ...
    var raidRecordingName = document.getElementById("raidRecordingName");
    var raidRecordingBtn = document.getElementById("raidRecordingButton");
...
function startRecordingRaid() {
    var raidNameElement = document.getElementById("raidRecordingName");
    var recordingButton = document.getElementById("raidRecordingButton");
    ...
    if (recordingButton.innerText == "Start") {
        if (raidName != "") {
            ...
            fs.writeFile(configuration["raid_name"], JSON.stringify(object, null, 4), function (err) {
                if (err) {
                    return;
                }
                PythonShell.run(configuration["raid_record_script"], null, function (err, results) {
                    if (err) {
                        throw err;
                    }
                    console.log(results);
...
map.addEventListener("mousemove", function (ev) {
    ...
    coordsDiv.innerHTML = "<p id=\"latText\" class=\"infoText\">" + lat + "&deg;" + Math.floor(latMin) + "'" + Math.floor(latSec) + "\" " + lattitudalDirection + " " + lng + "&deg;" + Math.floor(lngMin) + "'" + Math.floor(lngSec) + "\" " + longitudalDirection + "</p>";
});
...
function latlngFormat(latlng) {
    ...
    return {
        lat: Math.abs(lat) + "&deg;" + Math.floor(latMin) + "'" + Math.floor(latSec) + "\" " + lattitudalDirection,
        lng: lng + "&deg;" + Math.floor(lngMin) + "'" + Math.floor(lngSec) + "\" " + longitudalDirection,
    };
}
function trackMarkerClick(e) {
    ...
    // if (e.layer.type == "H") {
    engagementsContainer.selectTarget(tracksContainer.getTrackWithName(e.layer.track.name));
    // } else {
    //     engagementsContainer.cancelCurrentAction();
    // }
}
function battalionMarkerClick(e) {
    ...
    if (e.layer.bat.availability == 0) {
        ...
        engagementsContainer.selectEngager(battalionsContainer.getBattalionWithName(e.layer.bat.name));
    }
}
...
function drawActionLine(selectedEngager, selectedTarget, lineType) {
    if (selectedTarget == undefined || selectedEngager == undefined) {
        ...
        else if (lineType == "broke" ||
            lineType == "hold_fire" ||
            lineType == "ineffect" ||
            lineType == "cantco") {
            engagementsContainer.cancelActionFor(selectedEngager.name, selectedTarget.name);
        }
        else {
...
function applyHighLevelEngagement() {
    var actionSelector = document.getElementById("highLevelCommandSelect");
    ...
    var btn = document.getElementById("highlevelApplyButton");
    btn.disabled = true;
    if (data == undefined) {
            ...
            con.query(sql, function (err, result) {
                ...
                if (count == 0) {
                    ...
                    con.query(sql, function (err, result) {
                        ...
                        engagementsContainer.setActionLine(selectedEngager, selectedTarget, currentAction);
                        // console.log("Inserted Result: " + result);
                    });
                }
                else if (count > 0) {
...
function applyBattalionEngagement() {
    var actionSelector = document.getElementById("battalionCommandSelect");
    var actionID = undefined;
    currentAction = actionSelector.value;
    ...
    var data = engagementsContainer.getSelected();
    var btn = document.getElementById("battalionApplyButton");
    btn.disabled = true;
    if (data == undefined) {
        alert("Select track and battalion first");
        btn.disabled = false;
...
function applyBrigadeReplyEngagement() {
    var actionSelector = document.getElementById("brigadeReplyCommandSelect");
    var actionID = undefined;
    currentAction = actionSelector.value;
    ...
    var data = engagementsContainer.getSelected();
    var btn = document.getElementById("brigadeReplyApplyButton");
    btn.disabled = true;
    if (data == undefined) {
        alert("Select track and battalion first");
        btn.disabled = false;
...
var entireScreenRef = document.getElementById("EntireScreen");
var displayOptionsSidePanelRef = document.getElementById("displayOptionsSidePanel");
